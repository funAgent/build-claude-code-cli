...
export class StreamingToolExecutor {
   ...
   * 规则：
   * - 如果第一个工具不安全 → 单独执行（独占）
   * - 如果第一个安全 → 连续收集后续所有安全工具（批量并行）
   * - 遇到不安全工具时停止（保证写操作的执行顺序）
   *
   * 示例队列: [glob✅, read✅, read✅, write❌, grep✅]
   *   Batch1: [glob, read, read] → Promise.all
   *   Batch2: [write]            → 独占 await
   *   Batch3: [grep]             → 独占 await
   */
