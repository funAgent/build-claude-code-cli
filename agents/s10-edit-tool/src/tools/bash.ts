...
const DANGER = [/\brm\s+(-[a-zA-Z]*)?.*\s+\/(?!\w)/, /\bmkfs\b/, /\bshutdown\b/, /\breboot\b/];

async function exec(cmd: string, cwd?: string): Promise<{ stdout: string; stderr: string; exitCode: number }> {
  for (const p of DANGER) if (p.test(cmd)) return { stdout: "", stderr: `[安全拦截] ${p.source}`, exitCode: 1 };
...
export const bashTool = buildTool({
  ...
  async call(input, ctx): Promise<ToolResult> {
    ...
    return { output: r.stderr ? `${r.stdout}\nstderr: ${r.stderr}\nexit: ${r.exitCode}` : r.stdout || "(no output)", isError: r.exitCode !== 0 };
  },
});
