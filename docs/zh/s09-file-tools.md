...
> **Real agents read and write files, not just shell commands**

`[ Phase 2: 工具系统 ]` · 工具数: 3 · 代码量: ~200 行
...
- FileRead 工具：带行号、可分页读取文件
- FileWrite 工具：自动建目录、安全写入
- 路径安全校验——防止<abbr data-tip="攻击者通过 ../ 等路径穿越符号访问工作目录之外的文件。例如 foo/../../../etc/passwd 可能读出系统密码文件。防御方法是将路径 resolve 后检查是否在 cwd 范围内。">目录穿越</abbr>
- 为什么文件操作需要成为"一等公民"工具而不是用 bash 代替
...
npm install
cp .env.example .env
npm run dev "请读取 package.json 的前 20 行并总结 name 字段"
```
...
<details>
<summary>练习 4 参考答案</summary>

假设 `cwd` 为 `/home/user/projects/my-app`：

**用例 1：`foo/../../../etc/passwd`**
- `resolve(cwd, "foo/../../../etc/passwd")` → `/home/user/etc/passwd`（取决于路径层级）
- 如果路径更深：`resolve("/home/user/projects/my-app", "foo/../../../etc/passwd")` → `/home/etc/passwd`
- `resolved.startsWith(cwd)` → `false` → 拒绝访问
- 模型收到 `is_error: true`，知道这个路径不允许访问

**用例 2：`src/components/Button.tsx`**
- `resolve(cwd, "src/components/Button.tsx")` → `/home/user/projects/my-app/src/components/Button.tsx`
- `resolved.startsWith(cwd)` → `true` → 允许访问
- 正常读取文件内容并返回
...
`file_write` 改一行代码需要重写整个文件，浪费 token。下一课 **s10 编辑工具** 将实现精确的局部替换——只发变更的 delta。
